...
modelForm = paste('Volume ~',mod,'+ (1 | gr(SubjectID, dist= "student")) + (',mod,'| gr(ROI, dist="student"))')
...
outdir <- "results/ROIwise/Bayes_t1/"
...
save.image(file=paste0(outdir,"/results_t1.RData"))
...
rois <- c("Left Amygdala", "Right Amygdala", "Left Hippocampus", "Right Hippocampus")

df <- read.table(file = "results/ROIwise/Bayes_t1/Clinical_Trajectory3_post.txt", sep = "", header = TRUE)
...
intercept <- ggplot(df.long, aes(x = Y, y = as.numeric(reorder(index,p)), group = ROI, fill = p.plot)) +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Clincical trajectory at T1",
    fill = "P+")
...
ggsave('Fig_t1_ct.png', plot = plot, dpi = 600, height = 120, width = 180, units = "mm")
