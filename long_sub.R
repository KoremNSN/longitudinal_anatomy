...
df_amy <- read.csv('data/amygdala_PTSD_hem.csv')
df_hip <- read.csv('data/hippocampus_PTSD_hem.csv')

db_amy <- melt(df_amy,
               id.vars = c('sub', 'hem', 'Whole_amygdala', 'time', 'month', 'Clinical_Trajectory', 'Gender', 'Age'),
               variable.name = 'roi',
               value.name = 'volume')

db_hip <- melt(df_hip,
               id.vars = c('sub', 'hem', 'Whole_hippocampus', 'time', 'month', 'Clinical_Trajectory', 'Gender', 'Age'),
               variable.name = 'roi',
               value.name = 'volume')

db_amy['ROI'] <- paste(db_amy$roi,db_amy$hem, sep = '.')
db_hip['ROI'] <- paste(db_hip$roi,db_hip$hem, sep = '.')
...
colnames(db_amy2)[3] <- "whole_region"
colnames(db_hip2)[3] <- "whole_region"

db = rbind(db_amy2,db_hip2)
...
mod = '1 + time * Clinical_Trajectory + Age + Gender + whole_region'
...
df <- read.table(file = "results/ROIwise/Bayes/timeT3:Clinical_Trajectory3_post.txt", sep = "", header = TRUE)
...
intercept <- ggplot(df.long, aes(x = Y, y = as.numeric(reorder(index,p)), group = ROI, fill = p.plot)) +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "T3 interaction term",
    fill = "P+")

plot <- ggdraw(intercept) +
  draw_text("P+", x = .94, y = .92, size= 11) +
  draw_text("PTSD > remession", x = .31, y = 0.015, size = 11) +
  draw_text("Remission > PTSD", x = .74, y = 0.015, size = 11)

ggsave('Fig_interaction_term_s.png', plot = plot, dpi = 600, height = 120, width = 180, units = "mm")
